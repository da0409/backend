...
- 分支：codex/java-cpp-layered-mvp，从最新 origin/main 创建。
- BE-JAVA-01 已完成；旧仓库 /home/tinyblack/hackathon 未作修改。
...
- 仓库 Maven Wrapper 3.9.16；CMake、g++、ffmpeg 和 MySQL 可用。
...
- Java 完成账号、素材、胶囊、推荐、领取签到、回信和时间线；C++ JNI 执行真实业务距离计算。
...
- 18 项 Java 测试全部通过：16 项真实 HTTP/MySQL 集成测试、2 项分层/JNI 测试。
- CTest geo_boundaries：通过，含 299.9/300.1 米、跨日期线和非法坐标边界。
